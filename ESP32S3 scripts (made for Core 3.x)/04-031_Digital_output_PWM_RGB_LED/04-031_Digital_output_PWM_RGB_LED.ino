/*  04.030 - ESP32-S3 PWM RGB LED Example
 *
 *  This sketch demonstrates how to use PWM to drive a common anode RGB LED.
 *  Updated to work with the ESP32-S3 and the latest Arduino-ESP32 core (v3.x+).
 *
 *  Components
 *  ----------
 *  - ESP32-S3 Dev Kit
 ...
 *  Connections
 *  -----------
 *  ESP32-S3 Dev Kit |     RGB LED
 *  ------------------------------
 *        GND        |  Common Anode
 *        GPIO3      |  Red pin via resistor
 *        GPIO4      |  Green pin via resistor
 *        GPIO5     |  Blue pin via resistor
 *
 *  Notes
 *  -----
 *  - GPIO32/33/25 used in the original ESP32 are not available on the ESP32-S3.
...
// PWM-capable GPIOs for ESP32-S3
